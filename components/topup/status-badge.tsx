...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  const getLabel = () => {
    switch (status) {
        case 'INIT': return 'Menunggu Pembayaran';
  ...
  const getIcon = () => {
    switch (status) {
        ...
        case 'PAID': 
        case 'PROCESSING': return <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />;
        case 'INIT': return <Clock className="w-3.5 h-3.5 mr-1" />;
        default: return null;
    }
  };
