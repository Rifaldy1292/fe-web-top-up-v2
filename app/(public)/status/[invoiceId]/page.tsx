...
export default function StatusPage({ params }: { params: Promise<{ invoiceId: string }> }) {
  const { invoiceId: routeInvoiceId } = use(params);
  
  const { 
    invoiceId, transactionStatus, 
    userId, selectedProductSlug, selectedNominalId,
    checkTransaction
  } = useTopupStore();

  // If coming directly to this URL, hydrate store
  useEffect(() => {
  ...
  const nominal = product.nominals.find(n => n.id === selectedNominalId) || product.nominals[0];

  const handleCopy = () => {
    navigator.clipboard.writeText(routeInvoiceId);
        ...
        <div className="inline-flex items-center justify-center p-3 rounded-full bg-muted mb-4">
            ...
            {transactionStatus === 'SUCCESS' ? (
                ...
                <div className="w-12 h-12 bg-muted-foreground/20 rounded-full flex items-center justify-center">
                    <StatusBadge status={transactionStatus} className="border-none bg-transparent scale-150 p-0" />
                </div>
            )}
        </div>
        <h1 className="text-2xl font-bold mb-1">
            {transactionStatus === 'SUCCESS' ? 'Pembelian Berhasil!' : 'Status Transaksi'}
        </h1>
        <p className="text-muted-foreground text-sm">
            {transactionStatus === 'SUCCESS' ? 'Top up telah masuk ke akun anda.' : 'Pantau status transaksi anda secara berkala.'}
        </p>
      </div>
        ...
        <CardContent className="pt-6">
            <div className="bg-muted/50 rounded-xl p-4 flex items-center justify-between mb-6 border">
                <div>
                     ...
                     <p className="font-mono font-bold text-sm">{routeInvoiceId}</p>
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                     ...
                     <span className="font-mono font-bold text-sm">{userId || "-"}</span>
                </div>
